...
--- you can set wd.customParams.use_unit_weapon_charge=true to let it works automatically,
--- 
...
for wdid, wd in pairs(WeaponDefs) do -- set ChargeDataWDs
    ...
    if wdcp.use_unit_weapon_charge then
        ...
        if script_burst and script_burst_rate then
            ...
            --Spring.Echo("unit_weapon_charge.lua: loaded weapondef " .. tostring(wd.name) .. " wdid " .. wdid .. " chargeExtra: " .. script_burst-1 .. " canShiftChargePerFrame: " .. (1/script_burst_rate/30))
        else
            Spring.Echo("Warning: unit_weapon_charge.lua: Bad unit_weapon_charge def for weapondef " .. tostring( wd.name))
        end
    end
end
    ...
    if wpns then
        for wpnnum, value in pairs(wpns) do
            ...
            if ChargeDataWDs[wdid] then
                ...
                --Spring.Echo("unit_weapon_charge.lua: loaded unitdef " .. tostring(ud.name) .. " which has gbs weapon slot " .. wpnnum)
            end
        end
...
local function SetUnitWeaponChargeData(uid,wpnnum,chargeDataW,currentCharge,lastFrame,chargeExtra,canShiftChargePerFrame)
    ...
    if not chargeDataW then -- create a chargeDataW for unit weapon
        ...
        if chargeExtra==nil then
            Spring.Utilities.UnitEcho(uid,"Error: unit_weapon_charge.lua: SetUnitGoodBurstChargeData: While Creating, Missing chargeExtra for unit wpnnum " .. wpnnum .. " fn blocked")
            return
        end

        if canShiftChargePerFrame==nil then
            Spring.Utilities.UnitEcho(uid,"Error: unit_weapon_charge.lua: SetUnitGoodBurstChargeData: While Creating, Missing canShiftChargePerFrame for unit wpnnum " .. wpnnum .. " fn blocked")
            return
        end

        if lastFrame==nil then
            --Spring.Utilities.UnitEcho(uid,"Warning: unit_weapon_charge.lua: SetUnitGoodBurstChargeData: Missing lastFrame for unit wpnnum " .. wpnnum .. ", use gameframe")
            --Spring.Echo("Warning: unit_weapon_charge.lua: SetUnitGoodBurstChargeData: Missing lastFrame for unit")
    ...
    if ChargeDataUDHas then
        for _, v in pairs(ChargeDataUDHas) do
            ...
            if not ChargeDataWD then
                Spring.Echo("Error: unit_weapon_charge.lua: Wrong ChargeDataUDsHas for unitdefid " .. tonumber(unitDefId) .. " wpnnum: " .. tonumber(wpnnum) .. " wdid: " .. tonumber(wdid) .. " is not a good_burst_salvo")
                --break
            else
            ...
            if tb then
                for _, wpnnum in pairs(tb) do
                    ...
                    if wpnnum~=nil then
                        ...
                        Spring.Utilities.UnitEcho(uid,"Error: unit_weapon_charge.lua: Bad UnitRulesParam UWCHas for unit, UWCHas: " .. tostring(UWCHas))
                    end
                end
            else
                Spring.Utilities.UnitEcho(uid,"Error: unit_weapon_charge.lua: Bad UnitRulesParam UWCHas for unit, UWCHas: " .. tostring(UWCHas))
...
local function ProcessChargeData(uid,wpnnum, chargeDataW,reloadMult,f)
    ...
    --[=[
    Spring.Echo("DEBUG: unit_weapon_charge.lua: reloadFrameLast: " .. tostring(reloadFrameLast))
    Spring.Echo("DEBUG: unit_weapon_charge.lua: currentCharge: " .. tostring(currentCharge))
    ]=]
    ...
    if reloadFrameLast<0 then -- reload finish
        ...
        if currentCharge>0 then-- charge -> reload
            ...
            --[=[
            Spring.Echo("DEBUG: unit_weapon_charge.lua: reloadFramePerBurst: " .. tostring(reloadFramePerBurst))
            Spring.Echo("DEBUG: unit_weapon_charge.lua: canShiftChargePerFrame: " .. tostring(canShiftChargePerFrame))
            Spring.Echo("DEBUG: unit_weapon_charge.lua: canShiftCharge: " .. tostring(canShiftCharge))
            Spring.Echo("DEBUG: unit_weapon_charge.lua: ShiftCharge: " .. tostring(ShiftCharge))
            ]=]
