...
local GetModInfo=function (modname)
	return VFS.UseArchive(modname,function ()
		return VFS.Include("modinfo.lua")
	end)
end
...
local function BuildConfigData(modinfo)
	...
	local CONFIG_NAME=modinfo.shortname or modinfo.name
	local CONFIG_FILENAME = LUAUI_DIRNAME .. 'Config/' .. CONFIG_NAME .. '_data.lua'
	local myConfigData
	...
	if (chunk == nil) then
		...
		setfenv(chunk, tmp)
		myConfigData = chunk()
		if (not myConfigData) then
			myConfigData = {} -- safety
		end
		if (myConfigData.version or 0) < DATA_VERSION then
			myConfigData = {}
			myConfigData.version = DATA_VERSION
		end
	end

	if modinfo.shortname=="ZK" then
		return myConfigData
	end

	local dependsConfigData={}
	local depends=modinfo.depend
	for i, dependModName in pairs(depends) do
...
function widgetHandler:LoadConfigData()
	self.configData=BuildConfigData(
		--GetModInfo()
		VFS.Include("modinfo.lua")
	)
	--[=[
	local chunk, err = loadfile(CONFIG_FILENAME)
	if (chunk == nil) then
